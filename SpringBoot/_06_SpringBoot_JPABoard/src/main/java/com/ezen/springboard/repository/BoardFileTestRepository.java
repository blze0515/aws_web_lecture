...
import com.ezen.springboard.entity.BoardFileTestId;

public interface BoardFileTestRepository extends JpaRepository<BoardFileTest, BoardFileTestId> {
	//@Query: 원하는 쿼리를 작성할 수 있는 어노테이션
	//nativeQuery: true로 설정하면 원하는 대로 쿼리 작성, 메소드명도 JPA 규칙에서 벗어날 수 있다.
	//value: 쿼리 작성
	@Query(value="SELECT IFNULL(MAX(F.BOARD_FILE_NO), 0) + 1 FROM T_BOARD_FILE_TEST F WHERE F.BOARD_NO=:boardNo", nativeQuery=true)
